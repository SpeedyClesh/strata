...
export function TransferForm({
  ...
  async function performTransfer(pin: string): Promise<{ ok: boolean; error?: string }> {
    try {
      ...
      if (!res.ok) {
        return { ok: false, error: data.error ?? "Transfer failed. Please try again." };
      }
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <Button type="submit" size="lg" className="gap-2 bg-strata-green hover:bg-strata-green-deep">
      ...
      <PinConfirmDialog
        ...
        description={`Enter your PIN to send ${formatCurrency(numericAmount || 0, currency)}.`}
